...
var expGuiCondition = function (pObject, config) {
    ...
    */
    ...
    function check() {
        // ＪＲ予約サービスと学割乗車券が同時指定されている場合
        if (getValue("JRReservation") != "none" && getValue("studentDiscount") != false) {
            alert("エクスプレス予約と学割");
        }
    }
    ...
    */
    function viewConditionDetail() {
        ...
        if (agent == 1) {
            ...
            buffer += '<a class="exp_conditionCloseButton" id="' + baseId + ':conditionClose" href="Javascript:check();"><span class="exp_text" id="' + baseId + ':conditionClose:text">閉じる</span></a>';
            buffer += '</div>';
            buffer += '</div>';
        }
        buffer += '</div>';
    ...
    */
    function fixCondition() {
        ...
        conditionList_f[11] = getJRReservation()[0];//getValueIndex("JRReservation"/*, parseInt(conditionList_f[11])*/);
